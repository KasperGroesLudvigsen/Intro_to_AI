...
# Initialize these before you train and test the models
def train_test_regressor(x_train, x_test, y_train, y_test, description, regressor):
    """
    Procedure for training and test a model. 
    ...
    print("Model: {} \n RMSE: {} \n MSE: {}".format(description, rmse, mse))
...
print("Model: {} \n RMSE: {} \n MSE: {}".format("Baseline", baseline_rmse, baseline_mse))
...
# Random search SVR
#svr = SVR(kernel="linear") # the experiments above showed that the linear kernel was best
svr = SVR()
distributions = dict(C=uniform(loc=0, scale=4), 
                     epsilon=uniform(loc=0, scale=1),
                     kernel=["linear", "poly", "rbf", "sigmoid"])
rscv = RandomizedSearchCV(svr, distributions, n_iter=1)
...
rscv = RandomizedSearchCV(elastic_net, distributions, n_iter=1000)
search = rscv.fit(x_train, y_train) # experiments above showed unscaled data was best
search.best_params_
...
description = "Random forest w. random search optimization"
_ = train_test_regressor(x_train, x_test, y_train, y_test, description, rf_random.best_estimator_) 
...
_ = train_test_regressor(x_train_poly, x_test_poly, y_train, y_test, description, regressor)
...
for degree in degrees: 
    ...
    #poly = PolynomialFeatures(degree)
    #print(poly)
    #_ = train_test_regressor(x_train, x_test, y_train, y_test, description, poly)



random_grid = {"degrees" : degrees}

polynomial_reg_random = RandomizedSearchCV(
    estimator = poly, 
    param_distributions = random_grid, 
    n_iter = 1, 
    cv = 3, 
...
polynomial_reg_random.fit(x_train, y_train)

print("Polynomial regression best parameters: \n {}".format(polynomial_reg_random.best_params_))
...
_ = train_test_regressor(
    ...
    y_test, 
    description, 
    polynomial_reg_random.best_estimator_
    ) 
